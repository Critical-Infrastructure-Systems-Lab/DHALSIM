...
class Minitown(MiniCPS):
    ...
    def automatic_start(self):
        ...
        plc1_process = plc1.popen(sys.executable, "automatic_plc.py", "-n", "plc1", "-w", "1", stderr=sys.stdout, stdout=plc1_output )
        time.sleep(0.2)
        plc2_process = plc2.popen(sys.executable, "automatic_plc.py", "-n", "plc2", "-w", "1", stderr=sys.stdout, stdout=plc2_output )
        scada_process = scada.popen(sys.executable, "automatic_plc.py", "-n", "scada", "-w", "1", stderr=sys.stdout, stdout=scada_output )

        print "[*] Launched the PLCs and SCADA process, launching simulation..."
        plant = net.get('plant')

        simulation_cmd = shlex.split("python automatic_plant.py -s pdd -t minitown -o physical_process.csv -w 1")
