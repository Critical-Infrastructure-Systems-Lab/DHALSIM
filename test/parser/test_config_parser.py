...
def test_generate_plc_config_no_plcs(tmpdir, inp_data_fixture):
    c = tmpdir.join("config.yaml")
    cpa_file = tmpdir.join("test.yaml")
    cpa_file.write("some: thing")
    c.write("cpa_file: test.yaml\ninp_file: input.inp")
...
def test_generate_plc_config_plcs_empty(tmpdir, inp_data_fixture):
    c = tmpdir.join("config.yaml")
    cpa_file = tmpdir.join("test.yaml")
    cpa_file.write("plcs: []")
    c.write("cpa_file: test.yaml\ninp_file: input.inp")
...
def test_generate_plc_config_plcs_normal(tmpdir, inp_data_fixture):
    c = tmpdir.join("config.yaml")
    cpa_file = tmpdir.join("test.yaml")
    cpa_file.write("""plcs:
  - name: PLC1
